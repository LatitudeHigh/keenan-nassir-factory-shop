...
function initColorMap() {
    VAL_TO_COLOR = {
        0 : new Color(00,00,00),
        "1/64" : new Color(0,96,115),
        "1/32" : new Color(10,147,150),
        "1/16" : new Color(148,210,189),
        "1/8" : new Color(233,216,166),
        "1/4" : new Color(238,155,0),
        "1/2" : new Color(202,103,2),
        "1": new Color(187,62,3),
        "2": new Color(174,32,18),
        "3": new Color(155,34,38),
        "4": new Color(237, 197, 63),
        "5": new Color(237, 194, 46)
    };
}
...
function start() {
    ...
    grid = new Grid(4, 4);
    initColorMap();
    CELL_SIZE = (getWidth()- (4 * CELL_PADDING)) / 4
...
function gridInit() {
    ...
    rect.setColor(new Color(107,34,37));
    add(rect);
    ...
    for (var i = 0; i < 4; i++) {
        for (var j = 0; j < 4; j++) {
            ...
            cell.text.setColor(new Color(122,23,11));
...
function addRandomCell() {
    ...
    if (chance < 75) {
        cell.rect.setColor(VAL_TO_COLOR["1/64"]);
        cell.value = "1/64";
        cell.text.setText("1/64");
    } else {
        //rolled a four
        cell.rect.setColor(VAL_TO_COLOR["1/32"]);
        cell.value = "1/32";
        cell.text.setText("1/32");
    }
    ...
    var birthAnimation = function() {
        ...
        if (size == CELL_SIZE) {
            ...
            cell.text.setColor(new Color(127,127,121));
...
//helps move the numbers in the game with the arrow keys
...
//this combines the math fractions together 
function combineFractions(cellValue) {
    if(cellValue.includes("/")) {
        var denominator = parseInt(cellValue.split("/")[1]) / 2;
        if(denominator == 1) {
            return "1";
        } else {
        ...
        return parseInt(cellValue) * 2 + "";
    }
}

function combineCells(cellSingle, cellDouble) {
    var doubleValue = combineFractions(cellSingle.value);
    cellSingle.value = 0;
    cellDouble.value = doubleValue;
    cellDouble.hasCombined = true;
    
    if (eval(doubleValue) > eval(highestValue)) {
        highestValue = doubleValue;
    }
}
//this is wear it shows how much scores you got 
            ...
            for (var i = 0; i < controllerList.length; i++) {
                ...
                if (controller.combines == true) {
                    ...
                    if (cellDouble.value > 4) {
                        cellDouble.text.setColor(Color.RED);
                    }
                    //update score
                    addToScore(eval(cellDouble.value));
                }
            }
            //add new cell
            if (highestValue == 30) {
